...
//    var employees: [Employee] = []
    ...
    @IBAction func removeEmployees(sender: NSButton) {
        let selectPeople: [Employee] = arrayController.selectedObjects as! [Employee]
        let alert = NSAlert()
        if selectPeople.count > 1 {
            alert.informativeText = "\(selectPeople.count) 个用户将被删除"
        } else {
            alert.informativeText = "\(selectPeople[0].name!) 将被删除"
        }
        
        alert.messageText = "你真的想删除嘛？"
        
        alert.addButtonWithTitle("移除")
        alert.addButtonWithTitle("取消")
        alert.addButtonWithTitle("警告")
        alert.addButtonWithTitle("1003")
        let window = sender.window!
        alert.beginSheetModalForWindow(window, completionHandler: { (response) -> Void in
            switch response {
                // NSAlertFirstButtonReturn 的值为 1000，每增加一个 button 值增加 1
                case NSAlertFirstButtonReturn:
                    self.arrayController.remove(nil)
                    print(response)
                case NSAlertThirdButtonReturn:
                    self.synth.startSpeakingString("keep, but no raise")
                case 1003:
                    print("1003")
                default : break
            }
